...
const _sfc_main = {
  name: "UniClouddb",
  setup(props) {
    const dataListRef = props.ssrKey ? props.getone ? common_vendor.shallowSsrRef(void 0, props.ssrKey) : common_vendor.ssrRef([], props.ssrKey) : props.getone ? common_vendor.shallowSsrRef(void 0, "5CDVFZ7JjLqAet+1TSPBSQ==") : common_vendor.ssrRef([], "9QomX/xMKqTmPCXK2MyyOQ==");
  ...
  methods: {
    ...
    add(value, {
      ...
      let db = common_vendor.Rt.database(this.spaceInfo);
      if (action) {
        db = db.action(action);
      }
    ...
    _execRemove(id, action, success, fail, complete, needConfirm, needLoading, loadingTitle) {
      ...
      const db = common_vendor.Rt.database(this.spaceInfo);
